test_that("format", {
  ...
  expect_error(calculate_sf(par_ref = fit.0,
  ...
  expect_error(
    calculate_sf(
      par_ref = matrix(data = fit.0$coefficients[,1],
                       ncol = 2),
      par_treat = matrix(data = rep(fit.4$coefficients[,1],times = 3),
                         ncol = 2)))
  expect_equal(
    length(
      calculate_sf(
...
test_that("robust calculation", {
  ...
  fit.0 <- pwr_reg(seeded = D$`Cells seeded`,D$`0 Gy`)
  fit.4 <- pwr_reg(seeded = D$`Cells seeded`,D$`4 Gy`)
  expect_equal(calculate_sf(par_ref = fit.0, par_treat = fit.4),
               calculate_sf(par_ref = matrix(data = fit.0$coefficients[,1],
                                             ncol = 2),
                            par_treat = matrix(data = fit.4$coefficients[,1],
                                               ncol = 2)))
  fit.0$coefficients[2, 1] <- 0
  expect_equal(calculate_sf(par_ref = fit.0, par_treat = fit.4),rep(Inf,2))})
...
test_that("correct calculation", {
  ...
  expect_equal(
    ...
    c(1,1))
  expect_equal(
    calculate_sf(par_ref = matrix(data = c(2,1,2,1),ncol = 2,byrow = TRUE),
