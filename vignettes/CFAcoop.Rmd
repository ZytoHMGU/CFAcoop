...
replicates of identical number of cells seeded, which are plotted as dots.
The color indicates the treatment and links the numbers of colonies counted 
...
the target region of good experimental practice.
Dashed lines show regression lines with a slope of $b=1$ (at $log10(a)$ 
varying from 0 to 4) for orientation, so that deviation can be spotted easily.  
The dots in the treatment response curves correspond to the survival fractions
at $C = 5, 20$ an $100$, with error bars indicating the uncertainty bands 
calculated as $2.5\%$ and $97.5\%$-percentiles of the parametric bootstrap 
($N = 1000$) evaluating the covarince matrices of the power regressions.
...
Without the assessment of the cellular cooperation, conventional calculation 
of a $SF'$ value, corresponds to picking randomly a sample from the 
distribution shown in the histogram above.

A comparison of the range of this distribution and the calculated uncertainties
of the new method shows, that for non-cooperative cell lines as T47D, there is 
no big difference in this variability/uncertainty.
...
At strong cellular cooperation, the PE-based calculated $SF'$ value is heavily 
affected by this cellular cooperation and the treatment effect of interest gets
